...
// Canonical display order for surface groups
const SURFACE_ORDER = ["Walls", "Ceiling", "Trim", "Exterior", "Accent", "Other"];
...
function isLightColor(hex: string): boolean {
  ...
  const g = parseInt(clean.slice(2, 4), 16);
  const b = parseInt(clean.slice(4, 6), 16);
  // Perceived luminance (WCAG-adjacent weighting)
  return (0.299 * r + 0.587 * g + 0.114 * b) / 255 > 0.55;
}
...
/**
 * Renders a color swatch square. Falls back to a hatched placeholder
 * when the hex value is missing or invalid.
 */
const ColorSwatch: React.FC<SwatchProps> = ({ hex, size = 52, className = "", style }) => {
  const valid = isValidHex(hex);
  return (
    <div
      ...
      style={{
        ...
        height: size,
        backgroundColor: valid ? hex : "transparent",
        // Checkerboard pattern for invalid/empty
        backgroundImage: valid
          ? undefined
          : "repeating-conic-gradient(var(--border-subtle) 0% 25%, var(--bg-elevated-softer) 0% 50%)",
        backgroundSize: valid ? undefined : "12px 12px",
...
const PaintEntryCard: React.FC<PaintEntryCardProps> = ({ entry, onEdit, onDelete }) => {
  ...
  return (
    ...
    >
      ...
      {/* Main content */}
      <div className="paint-entry-body">
        {/* Row 1: surface badge + color name */}
        <div className="paint-entry-top">
          <span className="paint-surface-badge">
            {SURFACE_ICONS[entry.surface] ?? "🎨"} {entry.surface}
          </span>
        ...
        {(entry.brand || entry.product_line || entry.finish) && (
          <div className="paint-entry-meta">
            ...
            {entry.product_line && <span className="paint-meta-sep">{entry.product_line}</span>}
            {entry.finish && (
              <span className="paint-finish-badge">{entry.finish}</span>
            )}
            {entry.size && <span className="paint-meta-sep muted">{entry.size}</span>}
          </div>
        )}
        ...
        {/* Row 4: base code + barcode (tertiary) */}
        {(entry.base_code || entry.barcode) && (
          <div className="paint-entry-sub paint-entry-codes">
            {entry.base_code && (
              <span title="Base code">
                <span className="paint-entry-code-label">Base</span>
                {entry.base_code}
              </span>
            )}
        ...
        {/* Row 5: notes */}
        {entry.notes && (
          <p className="paint-entry-notes">{entry.notes}</p>
        )}
      </div>
...
const PaintEntryForm: React.FC<PaintEntryFormProps> = ({
  ...
  const merged = { ...EMPTY_ENTRY, ...initial };
  const [form, setForm] = useState<Omit<PaintEntry, "id">>(merged);
  const [hexError, setHexError] = useState(false);
  ...
  return (
    ...
    >
      <div className="paint-form-header">
        <h4>{isEditing ? "✏️ Edit Paint Entry" : "🎨 Add Paint Entry"}</h4>
        ...
        {onAiParse && (
          <button
            ...
            className="btn-ai-parse"
            onClick={onAiParse}
            title="Auto-fill fields from a photo of a paint can label"
          >
            📷 Parse from Label Photo
            <span className="btn-ai-badge">AI</span>
          </button>
        )}
      </div>
      ...
      <div className="paint-form-section">
        ...
        <div className="form-row paint-form-row-3">
          ...
          <div className="form-group">
            <label htmlFor="pf-hex">Hex Color</label>
            <div className="paint-hex-input-group">
              {/* Native color picker — hidden behind the swatch button */}
              <input
                ref={colorInputRef}
                type="color"
                value={isValidHex(form.hex_color) ? form.hex_color : "#ffffff"}
                onChange={(e) => {
                  set("hex_color", e.target.value);
                  setHexError(false);
                }}
              ...
              <div
                className="paint-hex-swatch-btn"
                style={{ backgroundColor: validHex ? form.hex_color : "transparent" }}
      ...
      <div className="paint-form-section">
        ...
        <div className="form-group">
          <label htmlFor="pf-tint-formula">
            Tint Formula
            <span className="paint-label-hint">
              Colorant codes &amp; amounts from the paint can label
            </span>
          </label>
          <textarea
            id="pf-tint-formula"
            ...
            style={{ fontFamily: "monospace", fontSize: "0.8rem" }}
          />
          <span className="form-group help-text">
            Copy exactly from the sticker on the paint can — useful if you need a rematch.
          </span>
        </div>
      </div>
      ...
      <div className="paint-form-section">
        ...
        <div className="form-group">
          ...
          <textarea
            ...
            value={form.notes}
            onChange={(e) => set("notes", e.target.value)}
            placeholder="Any additional notes about this color or application…"
            rows={2}
            disabled={isSaving}
          />
        </div>
...
const PaintColorsTab: React.FC<PaintColorsTabProps> = ({ location, onUpdate }) => {
  // Read existing entries from the location, casting from unknown JSON
  const rawEntries = (location as any).paint_colors as PaintEntry[] | undefined;
  const [entries, setEntries] = useState<PaintEntry[]>(rawEntries ?? []);
  ...
  const persist = async (next: PaintEntry[]) => {
    ...
    try {
      await updateLocation(location.id.toString(), {
        // The backend stores arbitrary JSON fields on the location object
        paint_colors: next,
      } as any);
      setEntries(next);
      onUpdate();
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Delete this paint entry? This cannot be undone.")) return;
    await persist(entries.filter((e) => e.id !== id));
  };
  ...
  const handleSave = async (data: Omit<PaintEntry, "id">) => {
    if (editingEntry) {
      ...
      // Add new with a fresh UUID
      const newEntry: PaintEntry = {
        ...data,
  ...
  /**
   * Sort entries by canonical surface order, then by color name.
   * Returns a list of { surface, entries } groups.
   */
  const grouped: { surface: string; entries: PaintEntry[] }[] = (() => {
    const map = new Map<string, PaintEntry[]>();
    for (const entry of entries) {
      if (!map.has(entry.surface)) map.set(entry.surface, []);
      map.get(entry.surface)!.push(entry);
    }
    // Sort groups by SURFACE_ORDER, then sort entries within each group
    return SURFACE_ORDER
      .filter((s) => map.has(s))
      .map((s) => ({
        surface: s,
        entries: map.get(s)!.sort((a, b) => a.color_name.localeCompare(b.color_name)),
      }))
      // Append any surface not in SURFACE_ORDER (defensive)
      .concat(
        Array.from(map.entries())
          .filter(([s]) => !SURFACE_ORDER.includes(s))
          .map(([s, e]) => ({ surface: s, entries: e }))
  ...
  return (
    <div className="paint-colors-tab">
      ...
      {/* Add / Edit form (inline, shown above the list when adding) */}
      {showForm && (
        <div id="paint-entry-form-anchor">
          <PaintEntryForm
            ...
            onCancel={handleCancel}
            isSaving={saving}
            // Stub for future AI integration hook:
            // onAiParse={() => { /* launch AI photo parser */ }}
          />
        </div>
      )}
      ...
      {entries.length > 0 && (
        <div className="paint-entry-list">
          {grouped.map(({ surface, entries: surfaceEntries }) => (
            <div key={surface} className="paint-surface-group">
              {useGroupHeaders && (
                <div className="paint-surface-group-header">
                  <span>
                    {SURFACE_ICONS[surface] ?? "🎨"} {surface}
                  </span>
                  <span className="paint-group-count">
