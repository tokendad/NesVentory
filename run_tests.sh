...
if command -v pytest &> /dev/null; then
    run_test_suite \
        ...
        "cd backend && PYTHONPATH=. pytest ../docs/nimmbott/tests/test_rfid_detection.py -v --tb=short && cd .."
else
...
if command -v pytest &> /dev/null; then
    run_test_suite \
        ...
        "cd backend && PYTHONPATH=. pytest ../docs/nimmbott/tests/test_rfid_integration.py -v --tb=short && cd .."
else
...
if command -v npm &> /dev/null; then
    run_test_suite \
        ...
        "npm run test -- docs/nimmbott/tests/QRLabelPrint.rfid.test.tsx --run 2>&1 | tail -20"
else
...
echo "  📄 docs/nimmbott/PHASE_3_TESTING_CHECKLIST.md"
echo ""
