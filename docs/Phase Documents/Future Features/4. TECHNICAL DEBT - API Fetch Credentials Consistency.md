...
NesVentory's frontend authentication uses HttpOnly JWT cookies (secure, prevents XSS), but approximately 60+ API functions in `src/lib/api.ts` use inline `...authHeaders()` instead of the secure `createFetchOptions()` helper. This works fine in same-origin production (Docker container), but silently fails in cross-origin scenarios (dev mode with separate Vite server, future multi-origin deployments).
...
// Set-Cookie: access_token=<jwt>; HttpOnly; SameSite=Strict; Secure
...
#### Pattern 1: Secure (uses createFetchOptions) - ~15 functions
```typescript
...
#### Pattern 2: Insecure (uses authHeaders) - ~60+ functions
```typescript
...
| **Cross-origin Vite dev** | Silently fails ❌ | Works ✅ |
...
Complete inventory of ~60+ functions in `src/lib/api.ts` missing `credentials: 'include'`:

### Location Management (8 functions)
...
### Document Management (2 functions)
...
### Phase 1: Enhance createFetchOptions (30 minutes)

Ensure `createFetchOptions()` handles all request types cleanly:
...
No changes needed - it already supports all cases (POST, PUT, DELETE, FormData, etc.)
...
| Enhance createFetchOptions | 30 min | Easy |
| Batch refactor | 3-4 hours | Medium (repetitive) |
| Testing & validation | 1 hour | Medium |
| **Total** | **5-5.5 hours** | **Medium** |
...
To prevent regression, add an ESLint rule:

### Setup (one-time, 15 minutes)

**1. Install ESLint plugin:**
```bash
npm install --save-dev eslint-plugin-no-auth-patterns
```

**2. Create custom rule in `.eslintrc.cjs`:**
```javascript
module.exports = {
  rules: {
    'no-auth-patterns/missing-credentials-include': [
      'warn',
      {
        // Flag fetch() calls that use authHeaders() but don't have credentials: 'include'
        pattern: /fetch\([^)]*\{\s*[^}]*\.\.\.authHeaders\(\)[^}]*\}/,
      },
    ],
  },
};
...
**3. Or add comment-based ESLint directive:**
```typescript
// In fetch calls that intentionally omit credentials (e.g., public endpoints)
const res = await fetch(`${API_BASE_URL}/api/auth/google/status`, {
  headers: { "Accept": "application/json" },
  // eslint-disable-next-line no-auth-patterns/missing-credentials-include
});
```
...
**Lines Changed:** ~200 (across ~60+ functions)
**Breaking Changes:** None
**New Dependencies:** None
**Migration Needed:** No (backward compatible)

**Before & After:**
- **Before:** 15 functions use `createFetchOptions()`, ~60+ use `authHeaders()` (no-op)
- **After:** All 75+ functions consistently use `createFetchOptions()`
